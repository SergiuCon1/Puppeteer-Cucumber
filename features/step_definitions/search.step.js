...
} = require("/Users/sergi/JavaScript/Puppeteer2/lib/commands.js");
...
When('user clicks day of week {int}', {
    timeout: 2 * 5000
}, async function (day) {
...
When('user clicks on the movie with id {string}', {
    timeout: 2 * 5000
}, async function (time) {
    await clickElement(page, `[data-seance-id="${time}"]`);
});

When('user clicks on a free slots {int}', {
    timeout: 2 * 5000
}, async function (slot) {
    const freeSlots = await page.$$("[class='buying-scheme__chair buying-scheme__chair_standart']:not(buying-scheme__chair_taken)");
