const { clickElement, getText, putText, clickSeveralElements, expectedTicketInfo } = require("./lib/commands.js");
const { currentDate } = require("./lib/Date.js");
...
describe("Verify booking of the tickets", () => {
  test("Booking one ticket - positive case", async () => {
  ...
  test("Booking several tickets - positive case", async () => {
    const movieTimeButton = await page.$$("[class='page-nav__day-week']");
    await movieTimeButton[2].click();
    await clickElement(page, "[data-seance-id='129']");
    await clickSeveralElements(page, "[class='buying-scheme__chair buying-scheme__chair_standart']");
    await clickElement(page, "[class='acceptin-button']");
    // const selectorCost = getText(page, ".ticket__cost");
    // expect(selectorCost).toEqual(cost);
    await clickElement(page, "[class='acceptin-button']");
    const expected = await page.$eval("img", n => n.getAttribute("src"));
    expect(expected).toEqual("i/QR_code.png");
    await expectedTicketInfo(page, "Логан", "19:00");
  });
